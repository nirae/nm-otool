...
int get_section_32(void *file, uint8_t nb, size_t file_size, int type)
{
    ...
    while (cmd_numbers--)
    {
        ...
        lc = (void *)lc + addr_32(((struct load_command *)(lc))->cmdsize, type);
    }
    return (TRUE);
}
...
char get_symbol_letter_32(struct nlist *symtab, void *file, size_t file_size, int type)
{
    ...
    {
        char c = get_section_32(file, symtab->n_sect, file_size, type);
        if (!(symtab->n_type & N_EXT))
            c -= 'A' - 'a';
        return c;
    }
    ...
    if (addr_32(seg->cmd, type) == LC_SYMTAB)
    {
        ...
        while (sym_numbers)
        {
            ...
            if (sym_data->n_value)
                ...
                ft_memset(tmp, ' ', sizeof(tmp) - 1);
            // get_section_64(file, sym_data->n_sect, file_size);
            ft_printf("%s %c %s\n", tmp, get_symbol_letter_32(sym_data, file, file_size, type), name);
            symtab += sizeof(struct nlist);
            sym_numbers--;
            i++;
        }
